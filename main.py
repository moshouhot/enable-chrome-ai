...
def enable_chrome_flags(local_state):
    ...
    experiments = local_state["browser"].get("enabled_labs_experiments", [])
    ...
    for flag in required_flags:
        ...
        experiments = [e for e in experiments if not e.startswith(flag_name + "@")]
    ...
    local_state["browser"]["enabled_labs_experiments"] = unique_experiments

    if modified:
        print("Enabled Chrome flags: glic, glic-side-panel")
...
def patch_local_state(user_data_path):
    ...
    if "variations_permanent_consistency_country" in local_state:
        if (
            isinstance(local_state["variations_permanent_consistency_country"], list)
            and len(local_state["variations_permanent_consistency_country"]) >= 2
        ):
...
def main():
    ...
    for version, user_data_path in version_and_user_data_path.items():
        print("Patching Chrome", version, '"' + user_data_path + '"')
